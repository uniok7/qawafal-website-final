...
// A product with no price is valid — the message simply omits the price line
// rather than claiming the item costs 0.000.
...
export function productOrderMessage(t, name, price, id) {
  ...
  const lines = [t.whatsapp.orderIntro, name];
  if (hasPrice(price)) lines.push(`${t.whatsapp.priceLabel}: ${Number(price).toFixed(3)} ر.ع`);
  lines.push(url);
  return lines.join("\n");
}
